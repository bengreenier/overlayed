...
import { ApplicationSchema } from './app/schemas/overlayed'
...
const allocMainWindow = () => {
  // configure settings
  settings.global.configure({
    location: path.join(app.getPath('appData'), app.getName(), 'settings.json'),
    readFile: (p, o) => {
      if (!existsSync(p.toString())) {
        writeFileSync(p.toString(), '{}')
      }
      return jsonfile.readFileSync(p, o)
    },
    schema: ApplicationSchema
  })

  // see if we have window settings, if so, use them
  const windowDims = settings
    .global
...
const allocSettingsWindow = () => {
  ...
  settingsWindow = new BrowserWindow(windowDims)
  ...
  settingsWindow.on('close', () => {
    ...
    settings
      ...
      .set(ApplicationSchema
        ...
        .properties
        .bounds
        .__path, settingsWindow.getBounds())
  })
